    ...
    default:
        if(isset($lesson_id)) {     //Окремий урок з курсу
            ...
            if(!isset($course_id)) {     //Усі курси сайту(це тут тимчсово, потім сюди треба додати пошук курсів, чи щось таке)
                ...
                $course_data = mysql_query("SELECT id,title,author_nick,author_id FROM courses WHERE banned='0'",$db);
            }
        else {
                ...
                if ($course_data_array == true) {
                    if ($course_data_array["banned"] == '0') {                                                     //Виводим данні з окремого курсу
                        $course_data = mysql_query("SELECT id,title,author_id,author_nick,description FROM courses WHERE id='$course_id'");
        ...
        if(isset($_SESSION["user_id"])) {     //Навіщо на сайті вхід, якщо він не використовується?
            if(isset($_POST["course_create_submit"])) {
                if(!empty($_POST["course_title"]) and !empty($_POST["course_description"])) {
                    $course_title = htmlspecialchars(mysql_real_escape_string($_POST["course_title"]));     //Обобляємо дані
                    $course_description = htmlspecialchars(mysql_real_escape_string($_POST["course_description"]));

                    $user_data_check = mysql_query("SELECT courses_count, max_courses_count, banned FROM users WHERE id='".$_SESSION["user_id"]."'");     //Дані користувача - поточна кількість курсів, максимальна, та статус аккаунту
                    $user_data_check_array = mysql_fetch_array($user_data_check);
                    if($user_data_check_array["courses_count"] < $user_data_check_array["max_courses_count"] and $user_data_check_array["banned"] != 1) {     //Якщо поточна кількість курсів меньша за максимальну - ідемо далі
                        ...
                        $course_create_query = mysql_query("INSERT INTO courses SET title='$course_title', author_id='$user_id', user_nick='$user_nickname', description='$course_description'");     //Створюємо курс

                        $created_course_id = mysql_insert_id();     //ID цього курсу

                        $new_user_courses_count = $user_data_check_array["courses_count"] + 1;     //Нова кількість курсів на аккаунті
                        $account_update_query = mysql_query("UPDATE users SET courses='$new_user_courses_count' WHERE id='$user_id'");     //Вставляємо оновлену кількість курсів в базу
        ...
        if(isset($_SESSION["user_id"])) {     //Створювати курси можуть лише авторизовані користувачі
            if(isset($_POST["lesson_create_submit"])) {     //Якщо є POST дані, обробляємо їх
                if(!empty($_POST["lesson_mcourse_id"]) and !empty($_POST["lesson_title"]) and !empty($_POST["lesson_content"])) {
                    ...
                    $lesson_title = htmlspecialchars(mysql_real_escape_string($_POST["lesson_title"]));
                    $lesson_content = htmlspecialchars(mysql_real_escape_string($_POST["lesson_content"]));     //Обробка даних

                    $course_data_check = mysql_query("SELECT author_id,banned FROM courses WHERE id='$lesson_mcourse_id'");
                    $course_data_check_array = mysql_fetch_array($course_data_check);     //Дістаємо дані про курс та обробляємо їх
                    ...
                    if($course_data_check_array != false) {     //Якщо ми нічого не отримали - нічого не робимо
                        if($course_data_check_array["author_id"] == $_SESSION["user_id"]) {     //Додавати уроки можна лише на свої курси
                            if($course_data_check_array["banned"] != 1) {     //На заблоковані курси уроки додавати заборонено
                                ...
                                $lesson_create_query = mysql_query("INSERT INTO lessons SET author_id='".$_SESSION["user_id"]."', mcourse_id='$lesson_mcourse_id', title='$lesson_title', content='$lesson_content', cdate='$date'");
                                //Додаємо урок...

                                $created_lesson_id = mysql_insert_id();     //ID щойно доданого уроку
